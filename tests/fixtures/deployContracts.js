...
async function deployContracts() {
    ...
    const ElkDexOracle = await ethers.deployContract("ElkDexOracle", [Tokens["Token1"].target, ElkFactory.target, 86400, 24])
    ...
    const FarmFactory = await ethers.deployContract("ElkV2FarmFactory", [[ElkDexOracle.target], Tokens["Token1"].target], 
        {libraries: {ElkV2FactoryHelper: FactoryHelper.target, ElkV2FactoryHelperPermissioned: FactoryHelperPermissioned.target}})
