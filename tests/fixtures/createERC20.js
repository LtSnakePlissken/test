...
async function createERC20() {
    ...
    Pairs.map(async (pair) => {
        ...
        await ElkRouter.addLiquidity(TokenA.target, TokenB.target, w, w, x, x, deployer.address, (await ethers.provider.getBlock('latest')).timestamp + 1000)
        ...
        Rest.map(async (wallet) => {
            ...
            await (ElkRouter.connect(wallet)).addLiquidity(TokenA.target, TokenB.target, w, w, x, x, wallet.address, (await ethers.provider.getBlock('latest')).timestamp + 1000)
