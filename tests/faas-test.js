...
describe("Faas Test", function () {
    ...
    describe("Scenario 1", function () {
        ...
        before(async () => {
            ...
            PairA = new ethers.Contract(await ElkFactory.getPair(Token1.target, Token2.target), ElkPairData.abi, ethers.provider)
        ...
        it("Should successfully create a farm with various combinations of fees.", async () => {
            ...
            while (withdrawalFeeSchedule.length < schedule) {
                ...
                if (lastBps == maxFee || lastSchedule == range) {break}

            }
    
            await expect(FarmFactory.createNewRewards(
                ElkDexOracle.target,
                PairA.target,
        ...
        it("Should deposit LP from multiple wallets - including the wallet the farm was created with.", async () => {
            ...
            await expect(FarmManager.startEmission(farm, rewards, duration)).to.emit(farmContract, "RewardsEmissionStarted").withArgs(rewards, duration)
                ...
                switch(selected) {
                    ...
                    case 0:
                        ...
                        it("Should call getRewards() with multiple wallets.", async () => {
                            ...
                            if (await farmContract.rewards(Token1.target, deployer.address)) {
                                await expect((farmContract.connect(deployer)).getRewards(deployer.address)).to.emit(farmContract, "RewardPaid")
                            } else {
                    ...
                    case 1:
                        ...
                        it("Should withdrawl LP and deposit again with multple wallets.", async () =>{
                            ...
                            .to.emit(farmContract, "Withdrawn").withArgs(deployer.address, lpBalance - withdrawlFee)
                            ...
                            for (let i = 0; i < Rest.length; i++) {
                                ...
                                if (lpBalance) {
                                    ...
                                    .to.emit(farmContract, "Withdrawn").withArgs(Rest[i].address, lpBalance - withdrawlFee)
    ...
    describe("Scenario 2", function () {
        ...
        before(async () => {
            ...
            PairB = new ethers.Contract(await ElkFactory.getPair(Token3.target, Token4.target), ElkPairData.abi, ethers.provider)
...
async function getRewards(Rest, farmContract, Tokens) {
    ...
    for (let i = 0; i < Rest.length; i++) {
        ...
        if (earned) {
            ...
            await expect((farmContract.connect(Rest[i])).getRewards(Rest[i].address))
            .to.emit(farmContract, "RewardPaid")
