...
describe("FaaS Test", function() {
    ...
    before(async function () {
        ...
        this.FarmingRewards = await this.FarmingRewardsFactory.deploy(this.Oracle.address, _lpTokenAddress, _coverageTokenAddress, _coverageAmount, _coverageVestingDuration, 
            _rewardsTokenAddresses, _rewardsDuration, _depositFeeBps, _withdrawalFeesBps, _withdrawalFeesSchedule);
  ...
  describe("FaaS", function () {
    ...
    before(async function () {
      ...
      this.FactoryHelperPermissioned = await (await hre.ethers.getContractFactory("ElkFactoryHelperPermissioned")).deploy();

      this.FarmFactory = await (await hre.ethers.getContractFactory("ElkFarmFactory", {libraries: {
          ElkFactoryHelper: this.FactoryHelper.address, ElkFactoryHelperPermissioned: this.FactoryHelperPermissioned.address}})).deploy();
      
      this.FarmManager = await (await hre.ethers.getContractFactory("FarmManager")).deploy(this.FarmFactory.address);

      this.rewardsTokenContract = new hre.ethers.Contract("0xc7198437980c041c805A1EDcbA50c1Ce5db95118", ERC20.abi, hre.ethers.provider);
    ...
    describe("FarmingRewards contract creation through FarmFactory", function () {
      ...
      describe("Calling createNewRewards", function () {
        ...
        it("Should pay the fee and create a new FarmingRewards contract using createNewRewards", async function () {
          ...
          let newContractTx = await this.FarmFactoryInstance.createNewRewards(this.Oracle.address, _lpTokenAddress, _coverageTokenAddress, _coverageAmount, _coverageVestingDuration, 
              _rewardsTokenAddresses, _rewardsDuration, _depositFeeBps, _withdrawalFeesBps, _withdrawalFeesSchedule);
      ...
      describe("New Contract Interaction", function () {
        ...
        describe("Starting Emissions", function () {
          ...
          it("Should be able to send rewards and start emissions when called from creator account", async function () {
            ...
            await this.FarmManagerInstance.startEmission(this.newContract.address, [this.normalizedElk, this.normalizedReward], 2592000); // reward tokens need to be in correct order
    ...
    describe("No Coverage", function () {
      ...
      it("Should pay the fee and create a new FarmingRewards contract without a coverage token", async function () {
        ...
        // create the new contract through the farmFactory
        let newContractTx = await this.FarmFactoryInstance.createNewRewards(this.Oracle.address, second_lp, "0x0000000000000000000000000000000000000000", 0, 86400, 
            _rewardsTokenAddresses, _rewardsDuration, _depositFeeBps, _withdrawalFeesBps, _withdrawalFeesSchedule);
    ...
    describe("Permissioned Farms", function () {
      ...
      it("Should pay the fee and create a new FarmingRewardsPermissioned contract", async function () {
        ...
        // create the new contract through the farmFactory
        let newContractTx = await this.FarmFactoryInstance.createNewPermissonedRewards(this.Oracle.address, second_lp, "0x0000000000000000000000000000000000000000", 0, 86400, 
            _rewardsTokenAddresses, _rewardsDuration, _depositFeeBps, _withdrawalFeesBps, _withdrawalFeesSchedule);
